(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('funcadelic'), require('ramda/src/lens'), require('ramda/src/view'), require('ramda/src/set'), require('ramda/src/lensPath'), require('ramda/src/lensIndex'), require('ramda/src/compose'), require('object.getownpropertydescriptors'), require('ramda/src/mergeDeepRight'), require('ramda/src/indexOf'), require('symbol-observable')) :
	typeof define === 'function' && define.amd ? define(['exports', 'funcadelic', 'ramda/src/lens', 'ramda/src/view', 'ramda/src/set', 'ramda/src/lensPath', 'ramda/src/lensIndex', 'ramda/src/compose', 'object.getownpropertydescriptors', 'ramda/src/mergeDeepRight', 'ramda/src/indexOf', 'symbol-observable'], factory) :
	(factory((global.Microstates = {}),global.funcadelic,global.R.lens,global.R.view,global.R.set,global.R.lensPath,global.R.lensIndex,global.R.compose,global.Object.getownpropertydescriptors,global.R.mergeDeepRight,global.R.indexOf,global.SymbolObservable));
}(this, (function (exports,funcadelic,lens,view,set,lensPath,lensIndex,compose,getOwnPropertyDescriptors,mergeDeepRight,indexOf,SymbolObservable) { 'use strict';

lens = lens && lens.hasOwnProperty('default') ? lens['default'] : lens;
view = view && view.hasOwnProperty('default') ? view['default'] : view;
set = set && set.hasOwnProperty('default') ? set['default'] : set;
lensPath = lensPath && lensPath.hasOwnProperty('default') ? lensPath['default'] : lensPath;
lensIndex = lensIndex && lensIndex.hasOwnProperty('default') ? lensIndex['default'] : lensIndex;
compose = compose && compose.hasOwnProperty('default') ? compose['default'] : compose;
getOwnPropertyDescriptors = getOwnPropertyDescriptors && getOwnPropertyDescriptors.hasOwnProperty('default') ? getOwnPropertyDescriptors['default'] : getOwnPropertyDescriptors;
mergeDeepRight = mergeDeepRight && mergeDeepRight.hasOwnProperty('default') ? mergeDeepRight['default'] : mergeDeepRight;
indexOf = indexOf && indexOf.hasOwnProperty('default') ? indexOf['default'] : indexOf;
SymbolObservable = SymbolObservable && SymbolObservable.hasOwnProperty('default') ? SymbolObservable['default'] : SymbolObservable;
...
var Monad = funcadelic.type(function (_Applicative) {
...
var Chain = function () {
  ...
  _createClass(Chain, [{
    ...
    value: function map$$1(fn) {
      return new Chain(funcadelic.map(fn, this.valueOf()));
    }
  }, {
    key: "flatMap",
    ...
    value: function filter$$1(fn) {
      return new Chain(funcadelic.filter(fn, this.valueOf()));
    }
  }, {
    key: "append",
    value: function append$$1(thing) {
      return new Chain(funcadelic.append(this.valueOf(), thing));
    }
  }, {
    key: "tap",
...
function prune(tree) {
  ...
  return funcadelic.map(function (node) {
    return funcadelic.append(node, {
...
function lensTree() {
  ...
  function get(tree) {
    return funcadelic.foldl(function (subtree, key) {
    ...
    if (current.length === 0) {
      ...
      return new Tree({
        ...
        children: function children() {
          return funcadelic.map(function (child, childName) {
...
function getPrototypeDescriptors(Class) {
  ...
  if (prototype && prototype !== getPrototypeOf(Object)) {
    return funcadelic.append(getPrototypeDescriptors(prototype), getOwnPropertyDescriptors(Class.prototype));
  } else {
...
function parameterized(Type) {
  ...
  var parameters = funcadelic.foldl(function (parameters, param, index) {
    if (typeof param === 'function') {
      var key = keys[index];

      if (key) {
        return funcadelic.append(parameters, _defineProperty({}, key, param));
      } else {
        return parameters;
      }
    } else {
      return funcadelic.append(parameters, param);
    }
...
var ArrayType = function () {
  ...
  _createClass(ArrayType, [{
    ...
    key: "filter",
    value: function filter$$1(fn) {
      return funcadelic.foldl(function (_ref, state, i) {
    ...
    value: function splice(startIndex, length, values) {
      ...
      var moved = funcadelic.map(prune, tree.children.slice(startIndex + length));

      function attach(index, tree) {
        return graft(funcadelic.append(tree.data.path, index), tree);
      }

      var _children = chain(unchanged).append(funcadelic.map(function (child, i) {
        return attach(i + unchanged.length, child);
      }, added)).append(funcadelic.map(function (child, i) {
...
function parameterized$1(Constructor) {
  ...
  return parameterized.apply(void 0, [Type].concat(_toConsumableArray(funcadelic.map(function (item) {
    return typeof item === 'function' ? toType(item) : funcadelic.map(toType, item);
  }, rest))));
}
...
function transitionsFor(Type) {
  ...
  return funcadelic.append(common, transitionFns);
}
...
var Values = funcadelic.type(function () {
  function Values() {
...
Values.instance(Object, {
  values: function values(object) {
    return funcadelic.map(function (key) {
      return object[key];
    }, keys$3(object));
  }
});
...
var ContainsTypes = funcadelic.Monoid.create(function () {
...
function desugar(Type) {
  if (isSugar(Type)) {
    ...
    if (c === Array) {
      return parameterized$1.apply(void 0, [Array].concat(_toConsumableArray(funcadelic.map(desugar, values(Type)))));
    }

    if (c === Object) {
      return parameterized$1.apply(void 0, [Object].concat(_toConsumableArray(funcadelic.map(desugar, values(Type)))));
    }
  }
...
var Collapse = funcadelic.type(function () {
  function Collapse() {
...
function analyze(Type, value) {
  return flatMap(analyzeType(value), funcadelic.pure(Tree, new Node(Type, [])));
}
function collapseState(tree, value) {
  var truncated = truncate(function (node) {
    return node.isSimple;
  }, tree);
  return collapse(funcadelic.map(function (node) {
...
function analyzeType(value) {
  return function (node) {
    ...
    return new Tree({
      data: function data() {
        return Type === node.Type ? node : funcadelic.append(node, {
          Type: Type
        });
      },
      children: function children() {
        var childTypes = childrenAt(Type, node.valueAt(value));
        return funcadelic.map(function (ChildType, path) {
          return funcadelic.pure(Tree, new Node(ChildType, funcadelic.append(node.path, path)));
        }, childTypes);
      }
    });
  };
...
var Location = funcadelic.type(function () {
  function Location() {
...
Location.instance(Object, {
  stateAt: function stateAt(instance, value) {
    if (value) {
      return funcadelic.append(instance, value);
    } else {
      return instance;
    }
  },
...
Location.instance(types.Object, {
  ...
  childrenAt: function childrenAt(Type, value) {
    ...
    if (T !== any) {
      return funcadelic.map(function (_) {
        return T;
      }, value);
    } else {
...
function truncate(fn, tree) {
  return flatMap(function (node) {
    ...
    if (fn(subtree.data)) {
      return funcadelic.append(subtree, {
        children: []
      });
    } else {
      return subtree;
...
function cachedGetters(Type) {
  var descriptors = chain(getPrototypeDescriptors(Type)).filter(function (_ref2) {
    ...
    return funcadelic.append(descriptor, {
  ...
  _createClass(Node, [{
    ...
    value: function stateAt(value) {
      ...
      if (isSimple(Type)) {
        ...
        return _stateAt(Type, funcadelic.append(instance, cachedGetters(Type)), valueAt);
      }
    }
  }, {
    key: "transitionsAt",
    value: function transitionsAt(value, tree, invoke) {
      ...
      var Type = this.Type,
          path = this.path;
      return funcadelic.map(function (method) {
        return function () {
          for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
            args[_key] = arguments[_key];
          }
...
var Microstate$1 = function () {
  function Microstate(tree, value) {
    ...
    return funcadelic.append(funcadelic.map(function (transition) {
      return transition;
    }, this), {
      get state() {
        ...
        subscribe: function subscribe(observer) {
          ...
          function nextOnTransition(transition) {
            return function invoke() {
              var nextable = funcadelic.map(nextOnTransition, transition.apply(void 0, arguments));
              next(nextable);
              return nextable;
            };
          }

          next(funcadelic.map(nextOnTransition, microstate));
        }
...
funcadelic.Functor.instance(Microstate$1, {
  map: function map$$1(fn, microstate) {
    var _reveal = reveal(microstate),
        tree = _reveal.tree,
        value = _reveal.value;

    var next = funcadelic.map(function (node) {
      var transitions = node.transitionsAt(value, tree, invoke);
      return funcadelic.map(function (transition) {
        return function () {
    ...
    var mapped = funcadelic.map(function (transitions) {
      return funcadelic.map(fn, transitions);
    }, next);

    return funcadelic.append(microstate, collapse(mapped));
  }
});
Collapse.instance(Tree, {
  collapse: function collapse$$1(tree) {
    ...
    if (hasChildren) {
      return funcadelic.append(tree.data, funcadelic.map(function (child) {
...
funcadelic.Functor.instance(Tree, {
  map: function map$$1(fn, tree) {
    return new Tree({
      ...
      children: function children() {
        return funcadelic.map(function (child) {
          return funcadelic.map(fn, child);
        }, tree.children);
      }
    });
  }
});
funcadelic.Applicative.instance(Tree, {
...
exports['default'] = Microstate$1;
exports.create = create;
exports.reveal = reveal;
exports.Microstate = Microstate$1;
exports.Tree = Tree;
exports.analyze = analyze;
exports.parameterized = parameterized$1;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=microstates.umd.js.map
